...
BASE = "out/results"
OUT = "TIEMPO_CPLEX_MEJOR_ALGORITMO_POR_GRUPO.xlsx"
...
def summary_sheet(ws, note, value_of, note_height):
    ...
    for c in range(2, 3 + len(rows)):
        ...
        ws.cell(rt, c).value = f"=SUM({L}{r0}:{L}{rt-1})"
    ...
    for j in range(len(rows)):
        ...
        ws.cell(rt + 1, 3 + j).value = f"=B{rt}-{L}{rt}"
        ws.cell(rt + 2, 3 + j).value = f"=({L}{rt}-B{rt})/B{rt}*-1"
        ws.cell(rt + 2, 3 + j).number_format = "0.0%"
