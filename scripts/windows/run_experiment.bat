...
python scripts\analisis\generate_consolidated_report.py
if errorlevel 1 (
    ...
    python scripts\analisis\generate_comparative_charts.py
    if errorlevel 1 (
...
java -cp "%CLASSPATH%" -Djava.library.path="%CPLEX_LIB_PATH%" ec.Evolve -file src/model/params/%PARAMS_FILE% -p experiment.group=%G% -p jobs=%R% -p generations=100 -p pop.subpop.0.size=50 -p experiment.max.instances=8
...
python scripts\analisis\generate_excel_report.py %G%
if errorlevel 1 (
    ...
    python scripts\analisis\generate_charts.py %G%
    if errorlevel 1 (
