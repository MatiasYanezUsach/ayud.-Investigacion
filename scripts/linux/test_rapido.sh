...
if [ ! -f "$CPLEX_JAR_PATH" ]; then
    echo "ERROR: No se encuentra lib/cplex.jar en: $CPLEX_JAR_PATH"
    echo "Por favor, ajusta la ruta en este archivo"
    exit 1
fi

echo "[2/4] Compilando codigo..."
javac -cp "lib/commons-math3-3.6.1.jar:$CPLEX_JAR_PATH:ecj" -d bin src/model/*.java src/terminals/*.java src/functions/*.java
...
java -cp "bin:lib/commons-math3-3.6.1.jar:$CPLEX_JAR_PATH:ecj" \
     -Djava.library.path="$CPLEX_LIB_PATH" \
     model.CplexBaselineRunner \
     -instances "data/legacy/class1/C101_20_02.txt" \
     -output "out/test" \
     -timeLimit 5.0
