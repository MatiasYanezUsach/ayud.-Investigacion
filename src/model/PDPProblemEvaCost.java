...
public class PDPProblemEvaCost extends GPProblem implements SimpleProblemForm {
	...
	@Override
	public PDPProblemEvaCost clone() {
		PDPProblemEvaCost mispp = (PDPProblemEvaCost) super.clone();
		return mispp;
	}
	...
	public void evaluate(
		...
		if (!individual.evaluated) {
			...
			//to get the instance names and instance costs
			...
			for(int i = 0; i < data.size(); i++) {
				...
				currentProfit = auxData.get(i).getPDPInstance().getCurrentProfit();//solucinn obtenida

				instanceNames.add(auxData.get(i).getInstance().getName());
				instanceCosts.add(currentProfit);
			...
			try {
				FileIO.WriteCostEvaluation("out/results/evaluation/costEval.csv", instanceCosts);
			} catch (IOException e) {
				e.printStackTrace();
			}
