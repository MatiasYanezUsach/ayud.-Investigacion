...
public class PDPProblemEvaMean extends GPProblem implements SimpleProblemForm {
	...
	public static int repeatNumber = 30;
	public static double staticERPstd = 0.0;
	public static double meanHits = 0.0;
	public static double hitsStd = 0.0;
	...
	@Override
	public PDPProblemEvaMean clone() {
		PDPProblemEvaMean mispp = (PDPProblemEvaMean) super.clone();
		return mispp;
	}
	...
	{
		double[] nums = numArray.stream().mapToDouble(Double::doubleValue).toArray();
		StandardDeviation sd = new StandardDeviation(true);



		return Precision.round(sd.evaluate(nums),7);
	}

	public static double calculateMean(List<Double> numArray)
	{
		double[] nums = numArray.stream().mapToDouble(Double::doubleValue).toArray();

		Mean meanObj = new Mean();

		double mean = meanObj.evaluate(nums);


		return Precision.round(mean,7);
	}
	...
	public void evaluate(
		...
		if (!individual.evaluated) {
			...
			List<Double> erps = new ArrayList<>();
			List<Double> hitsRuns = new ArrayList<>();
			...
			for (int j = 0; j < repeatNumber; j++) {
				...
				for(int i = 0; i < data.size(); i++) {
					...
					gpind.trees[0].child.eval(state, threadnum, auxData.get(totalLoops), stack, gpind, this);    //evaluar el individuo gpind para la instancia i


					//fase reparatoria
					PDPInstance pdpi = auxData.get(totalLoops).getPDPInstance();
					...
					double bestProfit = PDPProblemEvo.Best_Profit(auxData.get(totalLoops).getInstance());//Obtiene el nptimo o la mejor solucinn conocida



					currentProfit = auxData.get(totalLoops).getPDPInstance().getCurrentProfit();//solucinn obtenida
					instanceRelativeError = PDPProblemEvo.Relative_Error(auxData.get(totalLoops).getInstance());//Error Relativo
					double err = bestProfit - currentProfit;
					List<ArrayList<Integer>> currentSol = auxData.get(totalLoops).getPDPInstance().getSol(); //solucinn obtenida
					...
					if (err == 0) {
						hits++; //instanceRelativeError < IND_MAX_REL_ERR ||
						runHits++;
						ListaHits.add(auxData.get(totalLoops).getInstance().getName());
					}
					//Time
					double duracion = (timeEnd - timeInit) / 1000000;//en milisegundos
					runErp += instanceRelativeError;
					...
					relErrAcum +=  Precision.round(instanceRelativeError,7);
					timeAcum += duracion;
					//System.out.println(auxData.get(i).getIndependenSet().getSol().subList(0, auxData.get(i).getIndependenSet().gettam_sol())+" / "+auxData.get(i).getIndependenSet().getSol().subList(auxData.get(i).getIndependenSet().gettam_sol(),(auxData.get(i).getIndependenSet().gettam_sol()+auxData.get(i).getIndependenSet().gettam_free()))+" - "+auxData.get(i).getIndependenSet().getSol().subList(auxData.get(i).getIndependenSet().gettam_sol()+auxData.get(i).getIndependenSet().gettam_free(), auxData.get(i).getIndependenSet().getnVert()));
					System.out.println(auxData.get(totalLoops).getInstance().getName() +" "+j +", currentProfit= " + currentProfit + ", ideal= " + bestProfit + ", Fitness= " + instanceRelativeError+ ", ERP= "+Precision.round(PDPProblemEvaMean.calculateMean(erps),7)+ ", ERPstd= "+String.format(Locale.US,"%.7f", Precision.round(PDPProblemEvaMean.calculateSD(erps),7)));
					totalLoops++;
					//System.out.println(auxData.get(i).getInstance().getName()+ " - Nodos : " + nodos + " ajuste 1: " + suma);
					//auxData.get(i).getIndependenSet().Print_ListAdya();

				}
				erps.add(Precision.round(runErp/repeatNumber,7));
				hitsRuns.add((double)runHits);
			}
			//FIN Evaluacinn del algoritmo con todas las intancias

			//Runtime garbage = Runtime.getRuntime();
			//garbage.gc();
			double ERP = Precision.round(PDPProblemEvaMean.calculateMean(erps),7);//Error Relativo Promedio
			double ERPstd =  Precision.round(PDPProblemEvaMean.calculateSD(erps),7);
			...
			f.hits = hits/repeatNumber;
			meanHits = Precision.round( PDPProblemEvaMean.calculateMean(hitsRuns),7);
			PDPProblemEvaMean.staticERPstd = ERPstd;
			PDPProblemEvaMean.hitsStd = Precision.round(PDPProblemEvaMean.calculateSD(hitsRuns),7);
			gpind.evaluated = true;
			float AdjustedFitness = (float) (1.0/(1.0 + Evo_fitness));
			double tiempoPromedio = timeAcum / totalLoops;
			...
			state.output.println("  ERP std = " + (float)ERPstd, LOG_FILE);
	...
	public void describe(final EvolutionState state,
		...
		state.output.println("label=\"Individual=" + indid + " MeanFitness=" + ((KozaFitness) gpind.fitness).standardizedFitness() +" FitnessStd="+ String.format(Locale.US,"%.7f",PDPProblemEvaMean.staticERPstd) +" MeanHits=" + PDPProblemEvaMean.meanHits + " Size=" + gpind.size() + " Depth=" + gpind.trees[0].child.depth() + "\";", DOT_FILE);
